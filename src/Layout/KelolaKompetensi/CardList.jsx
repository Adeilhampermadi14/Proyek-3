...
import { MEditKompetensi } from "./MEditKompetensi";
...
const [sessionExp,SetSessionExp] =useState(null) 
  const getKompetensi = async () => {
    await axios
      .post(
        `${process.env.REACT_APP_API_URL}/admin/kompetensi/search`,
  ...
  const deleteRuangan = async (id) => {
    await axios
      .delete(
        `${process.env.REACT_APP_API_URL}/admin/kompetensi/delete/${id}`,
        { headers: { Authorization: `Bearer ${Cookies.get("accessToken")}` } }
      )
      .then((res) => {
        dispatch(
