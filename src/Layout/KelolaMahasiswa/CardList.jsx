...
import { resetAlert, resetMore, setAlert } from "../../App/Features/AuthSlice";
import { MEditKelolaMahasiswa } from "./MEditMahasiswa";
...
export const CardList = ({ data }) => {
  ...
  const getMahasiswa = async () => {
    if (MoreClick === "MHS_NOT_VERIFY") {
      await axios
        .get(`${process.env.REACT_APP_API_URL}/admin/mahasiswa/n/verify`,{headers:{Authorization: `Bearer ${Cookies.get('accessToken')}`}})
        .then((res) => {
      ...
      await axios
        .post(`${process.env.REACT_APP_API_URL}/admin/mahasiswa/search`,{keyword:SearchResult},{headers:{Authorization: `Bearer ${Cookies.get('accessToken')}`}})
        .then((res) => {
          setCardS(res.data.data);
        })
        .catch((res) => {  if (res.response.status === 401) {
  ...
  const accMahasiswa = async (id) => {
    await axios
      .put(`${process.env.REACT_APP_API_URL}/admin/mahasiswa/acc/${id}`,{},{headers:{Authorization: `Bearer ${Cookies.get('accessToken')}`}})
      .then((res) => {
        dispatch(
  ...
  const deleteMahasiswa = async (id) => {
    await axios
      .delete(`${process.env.REACT_APP_API_URL}/admin/mahasiswa/delete/${id}`,{headers:{Authorization: `Bearer ${Cookies.get('accessToken')}`}})
      .then((res) => {
        dispatch(
          setAlert({ show: true, status: true, message: res.data.message })
        );
  ...
  return (
    ...
    {sessionExp && <OverleySessionOver /> }
      ...
      <CardRounded />
        ...
        cards.map((card) => (
          <>
          {clickEdit[card._id] && <MEditKelolaMahasiswa toggleClose={()=>toggleEdit(card._id)} data={card} />}
          <div
            key={card._id}
          ...
          >
            <div className="relative">
              ...
              >
                <ul className="py-1">
                  ...
                  <li
                    onClick={() => deleteMahasiswa(card._id)}
                    className="block px-4 py-2 text-[13px] text-gray-700 hover:bg-gray-100 cursor-pointer"
                  >
                    Hapus
                  </li>
            ...
            <div className="flex gap-3 w-full h-full items-center px-5">
              ...
              <ul className="font-[poppins] font-semibold text-[#252B48]">
                <li className="font-extrabold first-letter:uppercase">
                  {card.username}
                </li>
                <li className=" ml-2 first-letter:uppercase font-semibold">{card.mhs_name.length > 13 ? `${card.mhs_name.substring(0,13)}...`:card.mhs_name}</li>
                <li className=" font-reguler">{card.nim}</li>
              </ul>
            </div>
          </div>
