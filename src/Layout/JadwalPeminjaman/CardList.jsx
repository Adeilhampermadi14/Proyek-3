...
import { resetAlert } from "../../App/Features/AuthSlice";
...
export const CardList = ({ dateData,dataBack }) => {
  ...
  const getPermintaan = async () => {
    await axios
      .post(`${process.env.REACT_APP_API_URL}/mahasiswa/permintaan`, dateData, {
              ...
              currentPageData.map((res, index) => {
                return (
                  <tr className="text-[13px] border border-b-[1px] border-[#252B48]">
                    ...
                    <td className="px-2 break-words">{res.kompetensi}</td>
