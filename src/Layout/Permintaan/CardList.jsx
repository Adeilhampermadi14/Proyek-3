...
export const CardList = ({ data }) => {
  ...
  const getPermintaan = async () => {
    if (MoreClick === "PER_NOT_ACC") {
      await axios
        .get(`${process.env.REACT_APP_API_URL}/admin/permintaan/n/acc`, {
      ...
      await axios
        .post(
          `${process.env.REACT_APP_API_URL}/admin/permintaan/search`,
  ...
  const accPermintaan = async (id) => {
    await axios
      .put(
        `${process.env.REACT_APP_API_URL}/admin/permintaan/acc/${id}`,
        {},
        { headers: { Authorization: `Bearer ${Cookies.get("accessToken")}` } }
      )
      .then((res) => {
  ...
  const deletePermintaan = async (id) => {
    await axios
      .delete(
        `${process.env.REACT_APP_API_URL}/admin/permintaan/delete/${id}`,
        { headers: { Authorization: `Bearer ${Cookies.get("accessToken")}` } }
      )
      .then((res) => {
        dispatch(
                ...
                >
                  <ul className="py-1">
                    {card.status_permintaan ? (
                      ...
                      <>
                        <li
                          onClick={() => accPermintaan(card._id)}
                          className="block cursor-pointer px-4 py-2 text-[13px] text-gray-700 hover:bg-gray-100"
                        >
                          Terima
                        </li>
              ...
              <div className="flex gap-3 w-max h-full items-center px-5 mb-[60px]  ">
                ...
                <ul className="font-[poppins]  font-semibold text-[#252B48]  ">
                  <li className="font-bold first-letter:uppercase">
                    {card.mahasiswa.username}
                  </li>
                  <li> 🕵️‍♂️{card.dosen.nama_dosen.length > 13 ? `${card.dosen.nama_dosen.substring(1,13)}...`:card.dosen.nama_dosen }</li>
                  <li> 📖{card.kompetensi.nama_kompetensi.length > 13?`${card.kompetensi.nama_kompetensi.substring(1,13)}...`:card.kompetensi.nama_kompetensi}</li>
                  {/* <li> 📚Ukur Jarak</li> */}
                  <li> 🚪{card.ruangan.nama_ruangan.length >13 ? `${card.ruangan.nama_ruangan.substring(1,13)}...`:card.ruangan.nama_ruangan}</li>
                  <li> 🪑 Lantai {card.ruangan.lantai}</li>
                  <li> 📅{card.tanggal}</li>
                  <li>
 
                    🕑{card.jam.jam_mulai} - {card.jam.jam_selesai}
                  </li>
                </ul>
              </div>
            </li>
