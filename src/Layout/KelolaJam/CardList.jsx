...
export const CardList = () => {
  ...
  const getJam = async () => {
    await axios
      .post(`${process.env.REACT_APP_API_URL}/admin/jam/search`,{keyword:SearchResult},{headers:{Authorization: `Bearer ${Cookies.get('accessToken')}`}})
      .then((res) => {
  ...
  const deleteRuangan = async (id) => {
    await axios
      .delete(`${process.env.REACT_APP_API_URL}/admin/jam/delete/${id}`,{headers:{Authorization: `Bearer ${Cookies.get('accessToken')}`}})
      .then((res) => {
        dispatch(
  ...
  return (
    ...
    {sessionExp && <OverleySessionOver />}
      ...
      {!cards ? (
        ...
        cards.map((card) => (
          <>
            {clickEdit[card._id] && <MEditJam toggleClose={()=>toggleEdit(card._id)} data={card} />}
            <div
              key={card._id}
            ...
            >
              ...
              <div className="flex gap-3 w-full h-full items-center px-5">
                ...
                <ul className="font-[poppins] font-semibold text-[#252B48]">
                  <li className="font-bold first-letter:uppercase">{card && card.kategori}</li>
                  <li className=" first-letter:uppercase">{card.jam_mulai} - {card.jam_selesai}</li>
                </ul>
              </div>
            </div>
          </>
